...
class GridCanvas(QFrame):
    """Reusable grid-based canvas for placing panel cards."""
    ...
    def __init__(
        ...
        *,
        ...
        self._layout.setHorizontalSpacing(max(0, int(gap)))
        self._layout.setVerticalSpacing(max(0, int(gap)))
    ...
    def set_grid_config(
        ...
        *,
        ...
        The grid is discrete; changing the number of columns does not reflow
        existing placements. Prefer keeping `cols` stable (24) and only changing
        `rows`/`gap`.
        """
    ...
    def add_panels(self, descriptors: Iterable[PanelDescriptor]) -> None:
    ...
    def resizeEvent(self, event: Any) -> None:  # pragma: no cover - UI resize
    ...
    def _apply_row_stretches(self) -> None:
        ...
        # If the effective row count decreased, clear stale stretch factors.
        for row in range(target_rows, self._row_stretch_applied):
            self._layout.setRowStretch(row, 0)

        self._row_stretch_applied = max(self._row_stretch_applied, target_rows)


__all__ = ["GridCanvas", "PanelDescriptor"]
