...
NFLVERSE_PLAYERS_URL = (
    "https://raw.githubusercontent.com/nflverse/nflverse-data/master/players/players.csv"
)
...
def download_registry(config: AppConfig, console: Console, force: bool = False) -> Path:
    ...
    We deliberately keep this dependency light (plain `requests`) so the script can run
    even before the rest of the ingestion stack is ready.
    """
    ...
    console.log(f"[bold yellow]Fetching NFLverse registry ->[/] {cache_path}")
    response = requests.get(NFLVERSE_PLAYERS_URL, stream=True, timeout=60)
    ...
    with Progress(
        ...
        task_id = progress.add_task("download", total=total_bytes or None)
        with cache_path.open("wb") as f:
            for chunk in response.iter_content(chunk_size=8192):
                f.write(chunk)
                progress.update(task_id, advance=len(chunk))

    console.log("[bold green]Download complete[/]")
    return cache_path
